     ...
     for i:=1 to n do begin
       ...
       if FileVersion>=5 then begin
          ...
          for j:=1 to m do begin
            ...
            s.refexposure:=FSequenceFile.Items.GetValue('/Targets/Target'+inttostr(i)+'/Plan/Steps/Step'+inttostr(j)+'/RefExposure','');
    ...
    if t.starttime=-2 then begin   // really circumpolar on local horizon
      ...
      if (intime<=0) and (t.endtime>=0) then begin
         ...
         if stw>60 then begin
            ...
            if t.endset then
              msg(Format(rsObjectSetsAt, [FormatDateTime('hh:nn', t.endtime),FormatFloat(f1, stw/3600)]), 3)
            else
              msg(Format(rsObservationE, [FormatDateTime('hh:nn', t.endtime), FormatFloat(f1, stw/3600)]), 3);
